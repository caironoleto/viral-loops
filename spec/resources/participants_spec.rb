...
RSpec.describe ViralLoops::Resources::Participants do
  ...
  describe '#convert' do
    context 'with a successful response' do
      let(:valid_params) { { user: { firstname: 'Cairo', lastname: 'Noleto', email: 'cairo@kompra.app' } } }
      let(:valid_response) { { processing: true } }

      before do
        stub_request(:post, 'https://app.viral-loops.com/api/v3/campaign/participant/convert')
          .to_return(status: 200, body: valid_response.to_json)
      end

      it 'returns a participant' do
        participant = subject.convert(valid_params)

        expect(participant).to be_a(ViralLoops::Models::Participant::Convert)
        expect(participant.processing).to be_truthy
      end
    end
    ...
    context 'with a failure response' do
      ...
      it 'raises an error' do
        expect { subject.convert(user: { firstname: 'Cairo' }) }.to raise_error do |error|
          expect(error).to be_a(ViralLoops::ApiError)
          expect(error.status).to eq(500)
          expect(error.status_symbol).to eq(:internal_server_error)
          expect(error.body).to eq('{"message":"Something went wrong"}')
